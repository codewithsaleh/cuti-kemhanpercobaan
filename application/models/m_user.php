...
class M_user extends CI_Model
{
    ...
    {
        // Hitung semua user yang aktif (level 1 = pegawai, level 3 = super admin)
        $hasil = $this->db->query("SELECT COUNT(id_user) as total_user FROM user WHERE id_user_level IN (1, 3) AND is_active = 1");
        return $hasil;
    }
    ...
    public function update_user($id, $username, $password)
    {
        $this->db->trans_start();
        if (!empty($password)) {
            $this->db->query("UPDATE user SET username='$username', password=md5('$password') WHERE id_user='$id'");
        } else {
            $this->db->query("UPDATE user SET username='$username' WHERE id_user='$id'");
        }
        $this->db->trans_complete();
        if ($this->db->trans_status() == true)
            return true;
    ...
    public function reset_password_by_id($id_user)
    {
        $this->db->trans_start();

        $default_password = 'kemhan2025';
        $hashed_password = password_hash($default_password, PASSWORD_DEFAULT);

        $this->db->query(
            "UPDATE user SET password = ? WHERE id_user = ?",
            array($hashed_password, $id_user)
        );
    ...
    // Tambahkan function untuk verify password (buat login nanti)
    public function verify_password($password, $hashed_password)
    {
        return password_verify($password, $hashed_password);
    }

    public function update_password_only($id, $password)
    {
        $this->db->trans_start();

        if (!empty($password)) {
            // [BARU] Gunakan password_hash() bukan md5()
    ...
    // [BARU] Tambahkan method untuk get user by id

    public function get_all_pegawai_with_sisa_cuti()
    {
        $this->db->select('user_detail.nama_lengkap, user_detail.nip, user_detail.sisa_cuti');
        $this->db->from('user');
    ...
    public function get_admin_for_chat()
    {
        $this->db->select('user.id_user, user_detail.nama_lengkap');
        $this->db->from('user');
        $this->db->join('user_detail', 'user.id_user_detail = user_detail.id_user_detail');
    ...
    {
        $data = ['reset_token' => $token, 'reset_token_created_at' => date('Y-m-d H:i:s')];
        $this->db->where('id_user', $user_id);
        return $this->db->update('user', $data);
    }
    public function get_user_by_reset_token($token)
    {
        return $this->db->get_where('user', ['reset_token' => $token])->row_array();
    }
    public function reset_password($user_id, $new_password)
    {
        $this->db->trans_start();
        $this->db->where('id_user', $user_id);
        $this->db->update('user', ['password' => md5($new_password)]);
        $this->db->where('id_user', $user_id);
        $this->db->update('user', ['reset_token' => null, 'reset_token_created_at' => null]);
        $this->db->trans_complete();
        return $this->db->trans_status();
    }
    public function count_bawahan_by_atasan($id_atasan)
    {
        $hasil = $this->db->query(" SELECT COUNT(id_user) as total_user FROM user WHERE id_atasan = '$id_atasan' ");
        return $hasil;
    }
    public function get_all_bawahan($id_atasan)
    {
    ...
    {
        return $this->get_pegawai_by_id($id_user);
    }
}
