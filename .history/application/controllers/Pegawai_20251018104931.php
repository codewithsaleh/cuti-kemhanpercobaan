...
$data['users'] = $this->db->get()->result_array();
    ...
    public function tambah_pegawai() {
        ...
        if (!$this->session->userdata('logged_in') || !in_array($this->session->userdata('id_user_level'), [2, 3])) {
            redirect('Auth');
        }
        ...
        if ($this->input->method() === 'post') {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                // Data user_detail - PERBAIKAN: id_atasan menggunakan id_user_detail bukan id_user
                $user_detail_data = [
                ...
                // PERBAIKAN: Konversi id_atasan dari id_user ke id_user_detail
                $id_atasan_user = $this->input->post('id_atasan');
                if (!empty($id_atasan_user)) {
                    // Ambil id_user_detail dari user yang dipilih sebagai atasan
                    $atasan = $this->db->get_where('user', ['id_user' => $id_atasan_user])->row_array();
                    if ($atasan) {
                        $user_detail_data['id_atasan'] = $atasan['id_user_detail'];
                    }
                }

                // Data user - PERBAIKAN: Tanpa kolom id_atasan
                $user_data = [
                    'id_user' => $id_user,
        ...
        if ($this->input->method() === 'post') {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (!empty($id_atasan_user)) {
                    ...
                    $user_detail_data['id_atasan'] = null;
                }
                ...
                if ($this->db->trans_status() === FALSE) {
                    $this->session->set_flashdata('eror_edit', 'Gagal mengupdate pegawai!');
                } else {
